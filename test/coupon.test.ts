...
describe("Coupon discount calculations", () => {
	it("should calculate correct subtotal and VAT without coupons", () => {
		const state = cartReducer(
			getInitialState(),
			addItem({ product: getMockProduct(1, 10.0), quantity: 2 }),
		); // Subtotal: 20
	...
	it("should apply SAVE10 coupon (10% percentage discount)", () => {
		let state = cartReducer(
			getInitialState(),
			addItem({ product: getMockProduct(1, 100.0) }),
		); // Subtotal: 100
	...
	it("should apply SAVE20 coupon (20% percentage discount)", () => {
		let state = cartReducer(
			getInitialState(),
			addItem({ product: getMockProduct(1, 50.0) }),
		); // Subtotal: 50
	...
	it("should apply WELCOME coupon (fixed $5 discount on min order $20)", () => {
		let state = cartReducer(
			getInitialState(),
			addItem({ product: getMockProduct(1, 20.0) }),
		); // Subtotal: 20
