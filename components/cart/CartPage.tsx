...
import { X, Tag, Sparkles, Shield, ShoppingCart } from "lucide-react";
...
export function CartPage() {
  ...
  const handleApplyCoupon = () => {
    ...
    if (appliedCoupon) {
      setCouponError("Only one coupon can be applied at a time. Remove the current coupon first.");
      return;
    }
  ...
  if (!mounted) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center p-8 text-center animate-pulse">
        ...
        <h2 className="text-display-sm font-semibold text-ink mb-2">Loading your cart...</h2>
      </div>
    );
  }
  ...
  if (isEmpty) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-sm">
        <div className="w-full max-w-[28rem] bg-canvas border border-hairline rounded-xl p-8 flex flex-col items-center text-center shadow-level-3">
          ...
          <h2 className="text-display-sm font-semibold text-ink mb-3 tracking-tight">Your cart is empty</h2>
          <p className="text-body-sm text-mute mb-8 max-w-[20rem] leading-relaxed">
            Your shopping cart is currently empty. Explore our products and add items to get started with your order.
          </p>
    ...
    <div className="max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-display-lg font-semibold text-ink tracking-tight mb-2">Shopping Cart</h1>
            <p className="text-body text-body-md">Review your order and proceed to checkout</p>
          </div>
          <div className="hidden md:flex items-center gap-4 text-sm text-body">
            ...
            <span className="flex items-center gap-1 text-success">
              ...
              {discount > 0 ? `You save ${formatCurrency(discount)}` : 'Add a coupon to save'}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-canvas rounded-xl border border-hairline p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-body-lg font-semibold text-ink">Promo Code</h3>
            </div>
              ...
              <div className="mt-4 p-4 bg-canvas-soft rounded-lg border border-hairline">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-body-sm font-medium text-ink">Coupon Applied</p>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="sticky top-24 bg-canvas rounded-xl border border-hairline p-6 space-y-5">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-body-lg font-semibold text-ink">Order Summary</h3>
            </div>
...
function CartItem({ item }: { item: { product: { id: number; title: string; price: number; stock: number; thumbnail?: string; category: string }; quantity: number } }) {
  const dispatch = useAppDispatch();
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 text-body-sm">
            <span className="font-medium text-ink">{formatCurrency(product.price)} each</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center border border-hairline rounded-lg bg-canvas overflow-hidden">
              ...
              <span className="w-10 text-center text-body-md font-medium text-ink">{quantity}</span>
              <button
          ...
          <div className="flex items-center gap-2">
            <span className="text-body-sm text-body">
              Line Total: <span className="font-semibold text-ink">{formatCurrency(lineTotal)}</span>
            </span>
            <button
