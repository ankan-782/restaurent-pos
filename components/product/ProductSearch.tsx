...
import { Filter, RotateCcw, Search, Tag, X } from "lucide-react";
...
export function ProductSearch({ categories }: ProductSearchProps) {
	...
	const [prevCommittedSearch, setPrevCommittedSearch] =
		useState(committedSearch);
	...
	const [prevCommittedCategory, setPrevCommittedCategory] =
		...
		<div className="bg-canvas rounded-lg border border-hairline p-4 space-y-4">
			<div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
				...
				<div className="relative flex-1">
					...
					<div className="relative">
						<Search
							className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-mute"
							aria-hidden="true"
						/>
						<Input
							id="search-input"
				...
				<div className="relative w-full sm:w-64">
					...
					<div className="relative">
						...
						>
							<SelectTrigger
								id="category-filter"
								className="flex items-center justify-between pl-11 pr-4 h-11 cursor-pointer"
							>
			...
			{hasActiveFilters && (
				<div className="flex flex-wrap items-center gap-2 pt-3 border-t border-hairline animate-fade-in">
					<span className="text-body-sm text-mute mr-1">
						Active Filters:
					</span>
					{pendingSearch && (
						<span className="inline-flex items-center gap-1.5 bg-canvas-soft border border-hairline rounded-full px-3 py-1 text-sm">
							...
							<span className="font-medium text-body max-w-[150px] truncate">
								{pendingSearch}
							</span>
							<button
								onClick={handleSearchClear}
