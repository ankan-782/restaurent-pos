...
export function ProductCard({ product, isLoading }: ProductCardProps) {
	...
	const isWishlisted = useAppSelector((state) =>
		product ? selectIsWishlisted(state, product.id) : false,
	);
	...
	return (
		<Card className="flex flex-col h-full transition-all duration-200 hover:shadow-level-4 hover:-translate-y-0.5 group overflow-hidden border border-hairline">
			...
			<Link
				...
				className="block relative aspect-square overflow-hidden bg-canvas-soft w-full"
			>
				{product.thumbnail ? (
					<Image
				...
				{/* Discount Badge */}
				{discountPercent > 0 && (
					<span className="absolute top-3 left-3 bg-error text-white text-caption font-medium px-2 py-1 rounded-sm z-10">
						-{discountPercent}%
					</span>
				)}
				...
				{mounted && (
					...
					>
						<Heart
							className={`h-4 w-4 transition-colors ${isWishlisted ? "fill-error text-error" : "text-body hover:text-error"}`}
						/>
					</button>
				)}
			</Link>
