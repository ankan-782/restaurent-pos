...
import { addItem, selectItems, updateQuantity } from "@/store/cartSlice";
import {
	ArrowLeft,
	CheckCircle,
	Package,
			...
			<div className="min-h-screen bg-background flex flex-col">
				...
				<main className="flex-1 max-w-7xl mx-auto w-full px-sm lg:px-lg py-16 flex flex-col items-center justify-center text-center">
					...
					<p className="text-body text-body-md mb-6 max-w-[24rem]">
						We couldn&apos;t retrieve the details for this product.
		...
		<main className="min-h-dvh flex-1 max-w-7xl mx-auto w-full px-sm lg:px-lg py-8">
			...
			<div className="grid md:grid-cols-2 gap-8 lg:gap-12">
						...
						<div className="flex gap-2 overflow-x-auto p-2 -m-2">
							{product.images.map((image, index) => (
								<button
									...
									className={`relative w-20 h-20 rounded-lg border bg-canvas shrink-0 cursor-pointer transition-all ${
				...
				<div className="flex flex-col">
					...
					<div className="border-t border-hairline pt-6 mb-6">
						<h3 className="text-body-sm-strong text-ink uppercase tracking-wide mb-2">
							Description
						</h3>
						<p className="text-body text-body-md leading-relaxed">
							{product.description}
						</p>
					</div>
						...
						{!mounted ? (
							...
							<div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
								<div className="flex items-center justify-between border border-hairline rounded-md bg-canvas overflow-hidden w-full sm:w-40">
									...
									<span className="text-body-md font-semibold text-ink text-center flex-1">
										{cartQuantity}
									</span>
									<button
								...
								<Link
									href="/cart"
									className="btn-primary flex-1 text-center justify-center gap-2"
								>
			...
			{product.reviews && product.reviews.length > 0 && (
				<div className="mt-16 border-t border-hairline pt-8">
					<h2 className="text-display-sm font-bold text-ink tracking-tight mb-6">
						Customer Reviews
					</h2>
					<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
						{product.reviews.map((review, index) => (
							<div
								key={index}
								className="bg-canvas border border-hairline rounded-xl p-6 shadow-level-1"
							>
								...
								<p className="text-body-md text-ink italic mb-4">
